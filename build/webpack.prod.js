...
if (config.buildConfig.staticPublicProjectPath && !process.env.isLib) {
  // 拷贝 asset 目录文件到 static-public 仓库
  let publicAssetPath = path.join(config.buildConfig.staticPublicProjectPath, 'asset/');
  if (!fs.existsSync(publicAssetPath)) {
    shelljs.mkdir('-p', publicAssetPath);
  }
  shelljs.cp('-r', path.join(config.paths.projectPath, 'asset/*'), publicAssetPath);
}
